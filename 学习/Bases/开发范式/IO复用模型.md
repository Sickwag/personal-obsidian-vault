...
ET 模式下不读取的数据会留在内核缓冲区中

| 后续发生的事件            | ET模式的行为                                             | 数据命运                                                  |
| ------------------ | --------------------------------------------------- | ----------------------------------------------------- |
| **1. 有新数据到达**      | **唤醒一次**：快递员再次来送新包裹，会**再次按铃**。                      | 你被唤醒后，**一次性把旧数据和新数据全部取走**（通常在一个while循环里读到`EAGAIN`为止）。 |
| **2. 你主动轮询**       | **不唤醒**：快递员不会主动上门，但你**可以主动去快递柜查看**（即程序主动调用 `read`）。 | 如果你知道可能有数据，就可以直接调用 `read`，把残留数据读走。                    |
| **3. 连接关闭（Close）** | **触发可读事件并唤醒**：对方关闭连接时，会触发一个`EPOLLIN`（读）事件，唤醒你。      | 你在处理关闭事件时，**最后读一次数据**，确保所有数据都被取走。                     |
