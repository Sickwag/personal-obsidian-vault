...
#### 数据类型
一个键存 512MB 数据
- string：二进制安全（保存任意数据），通过 `set/get` 操作，getbit/setbit 会直接操作字符串中的每一个字符（8 位），`set foo bar` 后，字符串被存储为：`01100010(b)01100001(a)01110010(r)`
...
- set，无序集合，`sadd/srem/spop/sdiff差集/sinter交集/scard` 操作，最大大小为 `2^32 - 1 (4,294,967,295)` 个成员
- zset，有序集合，基本命令同上，添加了一些 `zrank/zrevrank/zrange/zrevrange` 之类的排序查看操作
- stream 流，专门用于处理日志和事件流数据，作用类似于仅附加日志。您可以使用流实时记录和同步事件，Redis 为每个流条目生成一个唯一的 ID，通过 id 来操作流数据，使用 `X...` 流操作命令设置流大部分会返回流 ID
```redis
> - 将多个温度读数添加到流中
