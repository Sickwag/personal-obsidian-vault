...
- 集中式
    ...
    中央服务器存储文件, 每个主机得到的是文件副本, 主机修改完后上传到服务器.
...
提示使用 `git rm --cached 文件名` 拿回暂存区中文件
...
2. **1 file changed, 1 insertion (+)**:
    ...
    - `1 insertion(+)`: 表示在这些变化中，有 1 处是插入（即新增内容）。因是首次提交，所以这里的插入指的是你添加了一个新文件。
...
`git diff` 命令而不指定任何参数时，它默认比较的是**工作区中当前文件的状态**和**暂存区中该文件的最后状态**（也就是 add 后产生的文件快照）之间的差异。只能对比两个文件之间的差异
