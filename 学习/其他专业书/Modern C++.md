...
#### 移动语义的本质
C++11 的移动语义本质上是一套**所有权转让协议**。它的设计原则是：

> **"掏空一个对象"是一种破坏性操作，必须在代码中留下不可磨灭的痕迹。**

所以使用移动语义时必须要**显式声明这种破坏性操作**，`std::move` 本身**不移动任何东西**，它只是一个类型转换（`static_cast<T&&>`），把左值**强制转换为将亡值 (xvalue)**。它的全部意义在于**在源码层面留下意图证据**，让代码审查者和编译器都能清楚看到："此处发生了所有权转移"。
但这里有一个例外，[[#隐式移动语义]]
#### 隐式移动语义
当函数返回内部局部作用域空间中的变量作为返回值时，编译器会将这个值优化为右值，参考 [[C++开发范式和术语#RVO 和 NRVO]]
#### 注意事项
...
主板上多个内存芯片（Chip）共同组成连续存储空间，**CPU 以固定字长（如 4 字节/8 字节）为粒度访问内存**，4chip，8bank 的设计一般在 DDR3/4 上常见，现代 DDR5 内存一般使用更多的 chip 和 bank 数据，CPU 也对这种架构的内存有特殊优化
...
```
位域的限制：不能取地址、跨平台布局不确定（编译器可能从左到右或从右到左排列）、位域类型必须是整数类型。如果原类型需要的空间超过位域大小，会**截断并保留低位**，位域只能限制整形类型 + 枚举类型（本质上也是 int ）数据
#### linux \_\_cacheline_aligned_in_smp
```cpp
//  成员本身从缓存行边界开始，结构体内部插入填充
// [x前面有60字节填充][x][下一个缓存行][y][padding...56字节]   ← x从64字节边界开始
struct s{
	int a;
	int b __cacheline_aligned_in_smp;
};

// 整个结构体的起始地址对齐到缓存行边界（64字节）
// [x][y][padding...56字节]   ← 起始地址和结束对齐到64字节边界
struct s{
	int a;
	int b;
} __cacheline_aligned_in_smp;
```
注意，`__cacheline_aligned_in_smp` **不破坏结构体内存连续的性质**，本质上x，y 成员还是连在一起的，但在被读入缓存时的**截断位置改变了**
### 特殊情况
