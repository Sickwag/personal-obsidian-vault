...
### 布局大小设置
#### 窗口/控件大小
使用 `resize()` 设置窗口控件大小，`setSizePolicy()` 设置大小策略
```md
┌───────────┬──────────┬──────────────────────────────────────────────┐
│ Policy    │ 含义     │ 行为                                         │
├───────────┼──────────┼──────────────────────────────────────────────┤
│ Fixed     │ 固定大小 │ 不能拉伸也不能压缩，保持 sizeHint()          │
│ Minimum   │ 最小值   │ 可以拉伸，不能压缩到小于 minimumSize()       │
│ Maximum   │ 最大值   │ 可以压缩，不能拉伸超过 maximumSize()         │
│ Preferred │ 首选大小 │ 可以拉伸也可以压缩，但 sizeHint() 是最优大小 │
│ Expanding │ 扩展型   │ 强烈希望拉伸，可以压缩                       │
│ Ignored   │ 忽略     │ 完全忽略 sizeHint()，随意拉伸压缩            │
└───────────┴──────────┴──────────────────────────────────────────────┘
```
#### 文本控件的动态换行显示
`addWidget` 设置对齐方式和延展方式
```cpp
void addWidget(QWidget *w, int stretch, Qt::Alignment alignment)
```
`stretch` 参数
- 控制**多余空间如何分配**
- `stretch = 0` 表示使用 widget 的 `sizeHint()` 作为首选大小
- 值越大，分得的多余空间越多
`alignment` 参数
- 控制 widget 在**分配到的空间内如何对齐**
```cpp
leftSideLayout->addWidget(getColdNotice_, 0, Qt::AlignHCenter);
这样写时发生了什么：
1. stretch = 0
   ↓
   布局系统："这个 widget 想要 sizeHint() 那么多空间"
   ↓
   分配给 label 的宽度 = label->sizeHint().width() (固定值)

2. alignment = Qt::AlignHCenter
   ↓
   布局系统："让 widget 在它分配到的空间内居中"
   ↓
   但分配到的空间本身就是固定的！
结果：
┌─────────────────────────────────────┐
...
3. 没有 alignment 约束
   ↓
   布局系统："widget 可以填充整个可用宽度"
   ↓
   label 宽度 = 父布局宽度 (动态变化)

4. QLabel 的默认行为
   ↓
   宽度变化时，文本根据 wordWrap 重新排版

结果：
┌─────────────────────────────────────┐
│         leftSideLayout              │
│  ┌───────────────────────────────┐  │
│  │ getColdNotice (填充整个宽度)  │  │
│  │ 文本内容...                   │  │
│  └───────────────────────────────┘  │
└─────────────────────────────────────┘
```
窗口宽度变化时：
```md
情况 A: addWidget(label, 0, Qt::AlignHCenter)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
窗口宽： ┌────────┬──────────────────┐
        │ label  │     空白         │  ← label 宽度固定
        └────────┴──────────────────┘
...
窗口窄： ┌────┬──────────────────────┐
        │label│      空白           │  ← label 宽度还是固定
        └────┴──────────────────────┘


情况 B: addWidget(label)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
窗口宽： ┌────────────────────────────┐
        │       label (撑满)         │  ← 文本换行少
        └────────────────────────────┘
...
```
核心原因
**`QLabel` 的 `sizeHint()` 是基于其完整文本内容计算的**，当你使用 `AlignHCenter` 时，layout 会尊重这个 `sizeHint()`，不会给 label 更多空间。
解决方案总结

| 需求                                                 | 正确写法                                     |
| -------------------------------------------------- | ---------------------------------------- |
| 让 widget 填充父控件宽度并自动根据文本长度换行                        | `addWidget(widget)`                      |
| 让 widget 固定宽度居中（按照初始化窗口时计算的宽度设置 sizeHint()，之后不再改变） | `addWidget(widget, 0, Qt::AlignHCenter)` |
| 让 widget 按比例分配空间                                   | `addWidget(widget, stretchFactor)`       |
规则：**需要动态换行的 widget，不使用对齐策略下使用**。
### 信号与槽的使用
...
- 只要**头文件类**中包含 `Q_OBJECT` 宏，那么 MOC 就会为他生成对应的 `moc_classname.cpp` 文件，MOC**只会处理头文件中的 `Q_OBJECT` 宏**，如果将一个类定义在源文件中，那么 MOC**不会在预处理阶段展开宏，生成对应的元对象**，导致***编译时不报错，链接出错***。
...
- 接收要求：接收端必须绑定到相同的端口才能接收
- 网络可达性：接收端必须在**同一局域网段**内
...
#### 单播和广播使用 ip 地址不同细节
对于两个 UDP 发送程序，A 程序发送，B 程序接受，我在 A 程序中先后使用广播形式和单播形式发送了两条 hello 信息
A 程序端显示
```md
**已成功绑定
**绑定端口：3600
...
**已成功绑定
**绑定端口：1200
[From ::ffff: 172.26.192.1:3600] hello
[From ::ffff: 127.0.0.1:3600] hello
```
