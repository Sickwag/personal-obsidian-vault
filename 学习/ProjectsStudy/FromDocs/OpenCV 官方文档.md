...
opencv 中颜色构建方式：
- RGB是最常见的，因我们的眼睛使用类似的方式，但请记住OpenCV的标准显示系统使用BGR颜色空间（红色和蓝色通道互换位置）来组合颜色
...
- `CV_16SC1`：16 位有符号单通道
- `CV_32FC1`：32 位浮点单通道
构造函数中如果在前两个参数中填的是两个 int 类型参数，则表示当前 Mat 对象是一个二维对象，如果要实现多维图像，则需要在第一个参数填维度，第二个参数中填维度信息，比如 `2*2*2` 的三维空间就需要填一个 int 数组，并且内容为：`{2,2,2}`
#### 创建 mat 对象方法
```cpp
...
// 但不能使用，**`cv::Mat_` 不支持 C++11 初始化列表语法**（即 `{}`）
