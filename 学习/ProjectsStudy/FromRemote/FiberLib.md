...
### 数据结构和设计
**校时检测**，由于依赖系统的绝对时间，Sylar 特别设计了**时钟回拨 (Clock Rollover)** 的检测机制。如果系统时间被大幅度向后调整，调度器会强制触发所有定时器回调，以防止定时任务永久挂起。
受限于底层的 epoll_wait 系统调用，其超时精度通常维持在毫秒级。
**定时器与 IO 协程调度器的整合：**IO 协程调度器在空闲（Idle）状态下会阻塞在 epoll_wait 上等待网络 IO 事件。
- **超时动态调整**：在引入定时器前，epoll_wait 通常设置固定的超时时间（如 5 秒）。整合后，epoll_wait 的超时参数改用当前定时器管理器中**最小的超时剩余时间**。
- **唤醒逻辑**：epoll_wait 返回后，调度器会获取当前的绝对时间，将所有超时时间点早于当前时间的定时器收集起来（`listExpiredCallback()`），并将其回调函数推入任务队列执行。
**问题：epoll 唤醒一定代表定时器超时吗？** 答案是不一定。epoll_wait 返回的原因可能有两种：
- **IO 事件到达**（如套接字可读写）；
- **达到了设定的超时时间**（定时器到期）。因此，当 epoll_wait 返回时，不能理所当然地认为定时器已超时。此时，调度器需要再次比对当前绝对时间与定时器的目标时间，从而精准判断哪些定时任务需要被处理。
## IO Manager
