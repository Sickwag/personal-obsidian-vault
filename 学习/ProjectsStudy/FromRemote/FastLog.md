...
### 智能指针的 `reset()` 函数
智能指针中封装的类中最好不要定义 reset 函数，因智能指针对象也有一个 reset 函数，在我想调用封装类的 reset 函数时，容易写成 `ptr.reset()` 调用：
```cpp
...
- 为什么整个 FileLogBuffer 类使用模板？
  因缓冲区大小设计为了模板*非类型参数*，这样在**编译时确定大小，不再需要计算变长数组/字符串长度和动态分配内存**
- 为什么缓冲区需要使用 `std::array` 作为存储日志的实现？
  `std::array` 在栈上分配，`std::vector` 在堆上分配，栈分配内存更快，并且不需要动态分配内存，无数据迁移成本和带来的迁移不确定性（拓容后内存位置会变动）
...
- 为什么手动操作 array 中当期容量指针 `std::array<char, SIZE>::iterator __cur`？
...
- consteval 关键字修饰后的函数如果被调用了，它必须在编译期完成求值，**不能在运行时执行**
...
```
如果单独使用 std::format_string 通常是为了显式定义一个字符串中可以被填的参数类型，不允许修改，本质上只是将编译期的类型验证提前到代码编写阶段
### std::format_string
...
// 无法在编译时知道应该传什么类型的参数
std::format(fmt, "World", "not_a_number");  // 编译通过，但实际上传入不合理的值却不容易发现，因没有报错
```
- `std::format_string`：
明确指明我不希望这段格式化字符串只能格式化我想要的数据类型，如果不对就报错
```cpp
std::format_string<std::string, int> fmt = "Hello {}, age {}";  //包含参数类型信息
// 可以通过编译，但不能将这个对象传入std::format中，因第一个参数format_string<>尖括号中具体类型后面参数的类型推导后填
```
...
一个程序给多个日志文件输出日志情况是存在的，所以这里使用[[设计模式#生产者-消费者模式|生产者-消费者模式]]，生产者只管将写入需求发给消费者，消费者只管理缓冲区并将日志写入
- 生产者为 FileLogger 类中 log 函数，只管将日志内容写到三个缓冲区中，是使用日志库时被调用的对象，消费者无法被主动调用
- 消费者为 FileLogger 中 work 函数，是一个异步的线程
没有设计为两个类，因两者共享的资源在一个对象中，功能上也应该紧耦合，如果设计成两个类，那么**还需要一个类来作为消息队列管理缓冲区**，启动/停止消费者线程，对于原设计直接调用 log 函数显得很繁琐，并且日志包含 `std::string` 会有开销
#### 三重缓冲区设计
FileLogger 涉及文件读写，为减少磁盘 IO，设计三缓冲日志数据缓冲区
1. 当前缓冲区：当前正在写入日志的缓冲区
2. 满缓冲区列表：满了的缓冲区，通知等待被消费的线程
3. 空缓冲区列表：空的缓冲区，等待被填充，是满缓冲区中内容被写入文件后其中数据已经没用了，移动到空缓冲区
写入操作选择缓冲区的逻辑
