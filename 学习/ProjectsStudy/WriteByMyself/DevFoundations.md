...
- [[C++开发范式和术语#CRTP（Curiously Recurring Template Pattern）|CRTP]]，但这还是需要模板，在有纯虚函数的类中无法做到，需要重构通过继承 Task 重写 run 方法的提交任务逻辑。并且如果一个任务有多个不同类型的可能返回值就子类中需要写多个函数，或者子类函数使用模板。***总体流程过于复杂***
- `std::any` 最简单，C++17 支持
...
```
### Master-Slave 任务分配
参考 [[C++开发范式和术语#Master-Slave 任务分配机制]]
## 更简单的版本
### 实现 Fixed 模式
```cpp
// hpp file
...
### 3.2 RAII 与自动归还连接
[[#3.1 智能指针设计|3.1 智能指针设计]]中提到的归还连接需要通过**自定义 `shared_ptr` 的删除器**实现
```cpp
...
- 大于 512 字节对象由操作系统分配
	...
	C++对象模型的内存布局为：
- 小于由内存池管理
	`freeMemory` 函数中调用 delete 函数，会让系统级别内存管理器通过访问元数据位置部分的内容清楚对象的内存占用，如果调用 `operator delete(ptr, size)` (C++14)，显式指明块大小让编译器清理（**注意这并不会让编译器只清理 size 大小的内存**，参考[[#操作系统管理内存块逻辑|操作系统管理内存块逻辑]]），反而会降低效率。
### 理解测试
...
#### 1. `Common.h` - 基础工具类
- 提供内存对齐计算（`roundUp`）
- 将内存大小映射到自由链表索引（`getIndex`）
- 定义全局常量（对齐大小、最大内存等）
#### 2. `ThreadCache.h` - 线程本地缓存
**核心作用**：
- **线程独享**：每个线程有自己的缓存实例（`thread_local`）
- **无锁分配**：快速分配小对象（≤256KB）
- **批量管理**：当本地缓存不足时，从CentralCache批量获取
- **延迟归还**：避免频繁同步到CentralCache
#### 3. `CentralCache.h` - 中心缓存
**核心作用**：
- **线程间共享**：所有线程共享的中心缓存
- **批量传输**：一次给ThreadCache多个内存块（减少锁竞争）
- **锁粒度优化**：每个大小类有独立的自旋锁
- **延迟归还**：积累一定数量再批量处理
#### 4. `PageCache.h` - 页面缓存
**核心作用**：
- **大块内存管理**：以页（4KB）为单位分配内存
- **系统调用封装**：使用`mmap`直接分配大块内存
- **span合并**：释放时合并相邻空闲span，减少碎片
- **中央仓库**：为CentralCache提供大块内存
#### 5. `MemoryPool.h` - 统一接口
**核心作用**：
- **简化接口**：隐藏三级缓存实现细节
...
- 线程间竞争严重，性能下降
v2 的改进
- __线程本地缓存__：每个线程有自己的缓存实例（ThreadCache 对象），无锁访问
...
1. 非阻塞式线程调度
   与 `std::this_thread::sleep_for()` 不同，`yield()` 并不让线程真正"睡眠"或"等待"，而是主动放弃当前 CPU 时间片，告诉操作系统："我现在不紧急，可以让其他线程先运行"
2. 协作式多任务处理
   在多线程环境中，`yield()` 让线程**礼貌地让出 CPU 控制权**，而不是抢占式等待。这对于实现高效的忙等待（busy-waiting）模式特别有用。
其他线程等待方式：

| 场景       | 推荐替代方案                          | 说明                    |
| -------- | ------------------------------- | --------------------- |
| 等待条件变为真  | `std::condition_variable`       | 线程休眠直到被通知，CPU 占用率低[2] |
| 等待异步操作结果 | `std::future` 的 `get()`         | 阻塞等待异步任务完成[2]         |
| 暂停一段时间   | `std::this_thread::sleep_for()` | 精确控制暂停时间[2]           |
| 同步数据访问   | `std::mutex`, `std::lock_guard` | 提供线程安全保护              |
`yield()` 的具体行为依赖于操作系统和调度器实现，不同的操作系统调度策略会影响 `yield()` 的实际效果
- Windows: 调用 `SwitchToThread()`
- Linux: 调用 `sched_yield()`
- POSIX: 调用 `pthread_yield()`

最重要的注意事项是：**`yield()` 只是一个建议，不是强制要求**。操作系统的调度器可能会忽略这个提示，特别是：
- 没有其他就绪线程等待运行时
...
| 应用                                                | 跳表的作用             | 为什么用跳表           |
| ------------------------------------------------- | ----------------- | ---------------- |
| **Redis Sorted Set**，参考 [[Redis 设计与实现#第 5 章 跳跃表]] | 排行榜存储             | 实现简单、支持范围查询、易于并发 |
