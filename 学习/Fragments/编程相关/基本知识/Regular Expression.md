...
3. **字符类（Character Class）**
   - 用方括号 `[]` 表示，匹配方括号内的任意一个字符。例如，`[abc]` 匹配 `a`、`b` 或 `c` 中的任意一个字符。
...
7. **前瞻和后顾（Lookahead and Lookbehind）**
   - 用于声明一个必须存在但不包括在匹配结果中的断言。例如，`(?=abc)` 表示后面跟着 `abc` 的位置，`(?!abc)` 表示后面不跟着 `abc` 的位置。
...
- 也可以使用 `|` 来分组，表示**或**，但是 `|` 只会作用域两端的一个单位，`/P|pattern/` 是不能匹配大小写首字母的 pattern 的，pattern 是一个单位整体不可分，也可以理解为无连接符优先级高于 `|`
