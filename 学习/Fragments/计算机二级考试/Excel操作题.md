...
- 真题11
    ...
    - sumif函数日期限定方式
        ...
        不能在结束日不分青红皂白输入≤2012/1/31因你不知道一个月到底有几天，输入2012/2/31这种错误的日期（Excel不会自动调整）是就会直接输出错误的结果或者报错（错误的结果看不出来）所以使用起始等于，终止时间选择终止日期之后的一天，==并且小于不等于==
...
- 真题18
    ...
    - **关于链接多个条件**
        ...
        **注意：and函数返回的是一个true或false的结果，不能返回内容**
        ...
        比如现在要计算符合某种条件（也就是经过一层条件筛选）之后的一组数之中最大值
...
- 真题19
    ...
    - 真题20
        ...
        - 给表格设置保护
            ...
            - 理论上可以选定区域之后使用 允许编辑区域 这个选项锁定，但会扣分
...
- 2022年3月
    - 间断性重复填写内容
        ...
        - 使用Ctrl+G定位（选定所有的空值单元格，类似于Word中选定所有类似样式的内容）
            ...
            输入之后按下ctrl+enter表示对所有单元格（数组操作）使用公式，enter只会对选中单元格使用
