...
- 概念
    ...
    - smartart
        SmartArt 图形是信息和观点的视觉表示形式。可以通过从多种不同布局中进行选择来创建 SmartArt 图形，从而快速、轻松、准确地描述结构。就是一系列已经成型的表示某种关系的逻辑图，组织结构图。可以是并列、推理递进、发展演变，对比等。

        **A）通过插入SmartArt图形制作组织结构图
        B）直接在幻灯片的适当位置通过绘图工具绘制出组织结构图
        **
        C）通过插入图片或对象的方式，插入在其他程序中制作好的组织结构图

        **D）先在幻灯片中分级输入组织结构图的文字内容，然后将文字转换为SmartArt组织结构图
        A&D都是正确的，只不过A要优一点
        **

    - 幻灯片母版

        幻灯片母版用于设置幻灯片的样式，可供用户设定各种标题文字、背景、属性等，只需更改一项内容就可更改所有幻灯片的设计。在PowerPoint中有3种母版：幻灯片母版、讲义母版、备注母版。幻灯片母版包含标题样式和文本样式。在所有幻灯片中插入徽标，最优的方法就是使用幻灯片母版。当然也可以自己手动一张一张插入，但这个费时费事，每次调整时都需要一个一个调整。
        ...
        一般选择中提到对所有幻灯片进行统一操作时就需要用到幻灯片母版

    - 各种文件格式

        PPSX是放映文件(**只能放映**）格式，保存为该格式文件后，打开文件即开始放映；
        .POTX是模板文件（
        **只能套用**）格式，该格式可以将文件中设置好的格式，保存为模板，供以后直接调用该文件中格式使用；.
        PPTX是演示文稿格式（
        **能够编辑**），它打开后需要手动去设置播放。

        **将一个PowerPoint演示文稿保存为放映文件，最优的操作方法还是保存为.PPSX文件格式。**
...
- 查看幻灯片模式
    ...
    幻灯片的浏览视图中可以移动、删除、添加幻灯片（即对幻灯片的整理编排和顺序调整），浏览幻灯片，并在幻灯片下方显示编号。但不能修改幻灯片的内容，也不能调整格式，更不能对内容进行动画设计和设置播放效果
...
- 插入水印
    ...
    因每张幻灯片都需要水印，所以一般在幻灯片母版里面设置

    在Word中用艺术字，然后调整至相应的样式后保存为图片，导入PowerPoint中
...
- 调整smartart元素动画
    ...
    1. “整批发送”效果是一次性全部显示出来；
    2. “一次按级别”效果是，按照层次一层一层显示；
    3. “逐个按级别”效果是，按照层次组个显示；
    4. “逐个按分支”即“逐个”效果是按照一个一个显示，默认按照分支逐个显示，显示完一个分支后在显示下一个分支。
- 对不同类别的产品介绍
...
- PowerPoint编辑大纲
    ...
    [PowerPoint选择题](PowerPoint选择题.md)

    同Word也是使用标题来作为分割不同幻灯片的依据
    进入下一级文本输入状态：

    - <Ctrl+Enter>组合键可以快速进入到下一级文本输入状态。
    - <Shift+Enter>组合键是回车换行；按回车键
    - <Enter>是新建一张幻灯片。
    - 缩进效果也可以调整文本级别而不是一定要需要在**开始 选项卡中 选择 提升列表等级**
- 插入图片作为背景

    A）在幻灯片母版中插入该图片，并调整大小及排列方式
    B）将该图片文件做为对象插入全部幻灯片中
    C）将该图片做为背景插入并应用到全部幻灯片中



    注意无论是插入对象还是在母版中编辑，都是要手动调整图片的尺寸大小
