...
| C++11 | constexpr 函数隐式 inline（因为要能在 constexpr 上下文中使用） |
...
##### 全局作用域 inline 变量和函数
##### 局部作用域 inline 变量
##### 匿名命名空间下的 inline 变量和函数
##### 具名命名空间下的 inline 变量和函数
##### inline 语义总结

| 作用域               | `inline` 的语义     | 本质作用            |
| ----------------- | ---------------- | --------------- |
| 命名空间函数            | ODR 豁免 + 编译器提示   | **允许跨TU重复定义**   |
| 命名空间变量 (C++17)    | ODR 豁免 + 跨TU共享   | **允许在头文件中定义变量** |
...
class MyClass {
    ...
    // static const int count = 0;  // 3 这样是合法的，因是const且在类内定义
};
...
静态成员只能访问静态成员
##### 非匿名命名空间中的 static 符号
`static` 关键字用于命名空间层级（namespace scope）时，它的语义与在文件作用域的 `static` 一致：**将符号的内部链接性（internal linkage）强加给该名称**。命名空间是**逻辑分组机制**，解决的是名称冲突；static 是**链接性控制机制**，解决的是符号可见性。两者作用维度不同，但当它们放在一起时，static 的语义**覆盖**了命名空间的作用。
...
```
这样会导致不同的翻译单元由于 include 的纯复制粘贴本质都有一个 counter 实体，
##### 匿名命名空间中 static 符号
是一种冗余，匿名函数空间的出现就是 static 的上位替代
##### 总结
| 使用场景                                 | 作用                      | 示例                                 |
| ------------------------------------ | ----------------------- | ---------------------------------- |
...
#### inline 和 static 的配合
##### 匿名命名空间下的 inline static 变量和函数
##### 具名命名空间下的 inline static 变量和函数
##### 全局作用域下 inline static 变量和函数
##### 局部作用域 inline static 变量
##### 类成员 inline static
- 只有 static 成员才能够被设置为 inline
- （类内成员能不能被单独设置为 inline？不管成员函数还是变量）

##### 详细对比表：`static` / `inline` 在命名空间中的语义

| 写法                         | 链接属性                  | 跨 TU 共享？       | ODR 豁免？         | 实际含义                                                 |
| -------------------------- | --------------------- | -------------- | --------------- | ---------------------------------------------------- |
| **具名命名空间 `static`**        | **内部链接**              | ❌ 每个 TU 独立副本   | 不适用（不参与 ODR）    | **每个 TU 有自己的一份**，名字相同但地址不同。**C++11 已废弃**此用法          |
| **匿名命名空间 `static`**        | **内部链接**              | ❌ 每个 TU 独立副本   | 不适用             | `static` 多余，因为匿名命名空间已赋予内部链接。语法合法但无意义                 |
...
| **具名命名空间 `static inline`** | **内部链接**（`static` 胜出） | ❌ 每个 TU 独立副本   | ❌ 被 `static` 覆盖 | **最坏情况**：你期望跨 TU 共享（inline），实际每个 TU 独立（static）。行为易误解 |
