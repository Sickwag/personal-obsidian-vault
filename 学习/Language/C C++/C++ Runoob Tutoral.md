...
- 注意事项：
	...
	- 分离线程确实无法实现线程的重复利用，[[MyTinyTools#施磊手写线程池|线程池]]中一般不使用 detach 分离工作线程
```cpp
thread t([]() {
	cout << "Thread is running." << endl;
});
...
| C++20 | constexpr 变量/函数在特定条件下更灵活的 inline 规则 |
...
- `inline` 对此作用域无意义
**3. 类作用域**
- 类内定义的成员函数 → **隐式 `inline`**（C++98 起）。ODR 豁免，允许在头文件的类定义中写函数体
- 类外定义的成员函数 → 不加 inline 则不是 inline，ODR 需要遵守
...
- 类内成员函数 **可以单独设为 inline**（不管是不是 static 成员函数），效果：允许在多个 TU 中定义且链接器合并
**4. 匿名命名空间**
...
| 作用域 | `inline` 的语义 | 本质作用 |
| ----------------- | ---------------- | --------------- |
| 命名空间函数 | ODR 豁免 + 编译器提示 | **允许跨 TU 重复定义** |
...
| -------------------------- | --------------------- | -------------- | --------------- | ---------------------------------------------------- |
| **具名命名空间 `static`** | **内部链接** | ❌ 每个 TU 独立副本 | 不适用（不参与 ODR） | **每个 TU 有自己的一份**，名字相同但地址不同。**C++11 起废弃** |
...
- 对不同的类型强行转换可能会因为对象间不同的内存对其规则导致未定义行为
