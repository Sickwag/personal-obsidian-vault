...
- 父类对象转子类**原则上是不安全的**，只有确信**A 是 B 的父类时才能使用**，为了避免命名空间污染和作用域太宽泛问题，最好只在局部空间中使用。如果 A 不是 B 的父类，**指针或者引用转换是未定义行为，值传递会直接导致编译错误**。
...
- **和 C-style 转换一样，无关类型指针的转换会导致未定义行为，但不会bao**
注意：
`static_cast` 不能取消 const 或 volatile，一个 `const int` 类型变量不能被 `static_cast<int>` 取消 const 属性
如果类型不能被转换，会在编译期直接报错
除数值精度转换以外，**不产生额外开销**，只是将对象**重新解释为某种类型后复制到左值中**，如果是指针类型转换那不用复制，指针指向的地址不会变，地址前的元数据会变为转换类型
#### dynamic_cast
#### reinterpret_cast
#### const_cast
#### qobject_cast
