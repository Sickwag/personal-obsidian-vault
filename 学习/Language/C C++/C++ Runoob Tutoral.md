...
- lambda 在构建自身（**lambda 是一个对象，是对象就有成员，需要构造**）捕获列表中传入的自身名称是不完整的，传入对象为构建完，也就无法递归，这是可以有两种方法 ^cb8t8d
1. function 包装
...
和函数指针的对比-> [[#C++高级特性#函数对象 vs 函数指针 vs lambda]]
和包装器的对比-> [[#STL 资源库#functional 、lambda 和外部定义函数区别]]
## 数组
