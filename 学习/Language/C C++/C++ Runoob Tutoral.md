...
C++11 时，lambda 不能颁订模板参数成员函数，C++20 补上了这一问题，导致 bind 已经没有用武之地
```cpp
struct Processor {
    template<typename T>
...
- lambda 的仿函数内部实现机制让调用栈帧清晰简单，std::bind 由于有多层映射和 invoke 调用，还使用了模板会导致栈帧可读性非常差，名称非常长

> [!TIP]
> C++ 核心指南（C++ Core Guidelines）明确说 Favor lambdas over std::bind，Google 编码规范也禁止使用
  std::bind。对于 C++14 以上版本，无脑使用 lambda 即可
