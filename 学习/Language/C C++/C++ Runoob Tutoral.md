...
`for_each` 返回一个指向函数对象的迭代器，该迭代器指向最后一个被处理的元素之后的位置。通常这个返回值并不常用，因 `for_each` 的主要目的是执行操作而不是返回值。
...
- 因 x 直接引用数组元素，而不是创建它们的副本。所以这段代码运行完之后 myarray 中内容会被改变, 然而不加 `&` 符号就**不会引用, 而是创建副本**
...
- **创建对象之后**会执行所有构造函数函数体中代码块，这叫**初始化对象**
...
如果继承关系中不查虚函数表，那么子类重写父类的方法/实现父类虚函数之后，子类调用同名函数还是调用父类中，子类版本不会调用
### 数据抽象
...
所以遇到返回值根据函数参数列表决定的函数时，可以通过 auto 和 decltype 联合使用解决，但 decltype 只能放在后面，因参数 x，y 的作用域从函数调用之后开始，`}` 位置结束
```cpp
...
> [!note]
> 和互斥量的本质上不同是，互斥机制保证了线程之间可以出现竞态条件地访问/修改共享资源，而条件变量本质是为了让操作共享资源的代码**按一定顺序执行**，达到一定条件之后执行，这个条件由一个变量控制，所以叫做条件变量
...
```
对于野指针，`free` 之后指针指向的位置内容被清空指针没有置空，只能访问到这块内存的*随机指向*，因程序没有申请这块内存，所以无法修改
- 超出作用域的指针
```cpp
...
通过在插入前计算负载因子和桶数量，然后和大量插入之后的两者对比来判断是否触发了 `rehash()` 这一方法并不可靠，因：
