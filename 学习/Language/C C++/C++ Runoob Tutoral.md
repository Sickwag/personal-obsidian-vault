...
| 形参类型                  | 实参：左值 (lvalue)           | 实参：将亡值 (xvalue)           | 实参：纯右值 (prvalue) | 实参：带 `const` 的左值          |
| --------------------- | ------------------------ | ------------------------- | ---------------- | ------------------------- |
| **`T` (按值)**          | ✅ 拷贝构造                   | ✅ 移动构造                    | ✅ 移动构造           | ✅ 拷贝构造                    |
| **`T&` (左值引用)**       | ✅ 绑定                     | ❌ **编译错误**，将亡值销毁后导致悬空引用问题 | ❌ **编译错误**       | ❌ **编译错误**，将亡值销毁后导致悬空引用问题 |
| **`const T&`**        | ✅ 绑定                     | ✅ 绑定(延长寿命)                | ✅ 绑定(延长寿命)       | ✅ 绑定                      |
| **`T&&` (右值引用)**      | ❌ **编译错误**，右值引用仅能用于元编程场景 | ✅ 绑定(移动语义)                | ✅ 绑定(移动语义)       | ❌ **编译错误**                |
| **`template<T> T&&`** | ✅ 推导为 `T&`               | ✅ 推导为 `T&&`               | ✅ 推导为 `T&&`      | ✅ 推导为 `const T&`          |
#### CV 修饰符不一致按值传递 (Pass by Value: `T`)
- **形参带 CV，实参不带 CV**：合法。形参是实参的副本，形参带 `const` 只是保证函数内部不修改这个副本。
- **形参不带 CV，实参带 CV**：合法。发生拷贝构造，实参的 `const` 属性在拷贝后丢失，形参是一个普通的副本。
- _注意_：在函数签名中，**按值传递的顶层 CV 会被忽略**。`void f(int)` 和 `void f(const int)` 被视为同一个签名，不能用于重载。
#### CV 修饰符不一致按引用传递 (Pass by Reference: `T&`, `const T&` 等)
此时 CV 修饰符是**底层**的，规则非常严格：
- **形参带 `const`，实参不带 `const`**：**合法**。低权限向高权限转换是安全的（只读）。例如 `void f(const int&)` 可以接收 `int`。
- **形参不带 `const`，实参带 `const`**：**编译错误**。不能将 `const` 对象绑定到非 `const` 引用上，因为这会允许通过非 `const` 引用修改 `const` 对象，破坏了常量正确性。
- **关于 `volatile`**：逻辑与 `const` 完全一致。不能将 `volatile` 实参绑定到非 `volatile` 引用。
C++ 中表达式的值类别分为：**左值 (lvalue)**、**纯右值 (prvalue)**、**将亡值 (xvalue)**。其中，**右值 (rvalue) = 纯右值 + 将亡值**。
- **左值**：有身份，不可移动（如普通变量名 `x`）。
- **将亡值**：有身份，可移动（如 `std::move(x)` 的返回值）。
- **纯右值**：无身份，可移动（如字面量 `42`、临时对象 `Foo()`）。
#### 值类别不一致 形参是非 const 左值引用 (`T&`)
- **实参是左值**：**合法**。正常绑定。
- **实参是右值 (包含将亡值)**：**编译错误**。C++ 禁止将右值（临时对象或即将被移动的对象）绑定到非 const 左值引用，防止意外修改临时对象。
#### 值类别不一致 形参是右值引用 (`T&&`)
- **实参是左值**：**编译错误**。C++ 禁止将左值（具名对象）直接绑定到右值引用，防止意外“偷走”具名对象的所有权（必须显式使用 `std::move` 将其转为将亡值）。
- **实参是右值 (包含将亡值)**：**合法**。成功绑定，触发移动语义或延长临时对象生命周期。
#### 值类别不一致 形参是 const 左值引用 (`const T&`)
- **实参是左值**：**合法**。
- **实参是右值 (包含将亡值)**：**合法**。这是 C++ 的一个特例，`const T&` 可以绑定右值，并且会**延长临时对象的生命周期**，使其与引用的生命周期一致。
#### 值类别不一致 形参是按值传递 (`T`)
- 无论实参是左值还是右值，都**合法**。
- 如果实参是左值，调用**拷贝构造函数**。
- 如果实参是右值（将亡值/纯右值），优先调用**移动构造函数**（如果类型支持且满足noexcept），否则回退到拷贝构造。
#### 万能引用的特殊性
...
> [!note] 引用就是指针
> **引用就是指针**（绝大多数编译器实现中引用通过 `T* const` 常量指针实现），当传递一个引用时，编译器实际上是在寄存器或栈上压入了一个**指针（内存地址）**，在这时压入的"引用"是一个空指针，编译器会通过魔法优化这个引用类型，将他当作指针看待。这被称为*初始化这个引用*（即把实参的地址赋给这个底层指针），这在底层是有一条赋值指令的。所以，**“不需要构造”是不准确的，准确的说法是“不需要调用用户定义的拷贝构造函数”**。
