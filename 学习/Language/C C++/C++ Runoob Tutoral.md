...
> [!note] 引用就是指针
> **引用就是指针**（绝大多数编译器实现中引用通过 `T* const` 常量指针实现），当传递一个引用时，编译器实际上是在寄存器或栈上压入了一个**指针（内存地址）**，在这时压入的"引用"是一个空指针，编译器会通过魔法优化这个引用类型让他**作为一个已有符号的别名，不占用内存**，将他当作指针看待。这被称为*初始化这个引用*（即把实参的地址赋给这个底层指针），这在底层是有一条赋值指令的。所以，**“不需要构造”是不准确的，准确的说法是“不需要调用用户定义的拷贝构造函数”**。
...
### 大括号初始化
参考 [[C++ prime plus#用 `{}` 初始化转换]]和[[面试题合集#1992. 什么是 C++ 的列表初始化]]
#### 1. 基本语法形式
```cpp
