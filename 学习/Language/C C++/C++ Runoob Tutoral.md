...
静态成员只能访问静态成员
##### 非匿名命名空间中的 static 符号
`static` 关键字用于命名空间层级（namespace scope）时，它的语义与在文件作用域的 `static` 一致：**将符号的内部链接性（internal linkage）强加给该名称**。命名空间是**逻辑分组机制**，解决的是名称冲突；static 是**链接性控制机制**，解决的是符号可见性。两者作用维度不同，但当你把它们放在一起时，static 的语义**覆盖**了命名空间的作用。
...
```
这样会导致不同的编译单元由于 include 的纯复制粘贴本质都有一个 counter 实体，
##### 匿名命名空间中 static 符号
