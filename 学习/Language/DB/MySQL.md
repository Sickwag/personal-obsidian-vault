...
##### 流程控制函数
语法中所有的执行体（即判断成功后执行的动作，如 when 判断后的 then 后就是动作）如果是一个值表达式（返回一个值显示）那么条件句结尾不需要分号，如果是一个执行语句需要在每个操作后结尾使用分号
...
分组前筛选数据源是原始表，分组后筛选是筛选一遍后的结果（上一遍的结果表）放在 `having` 关键字中
##### 简单分组
```sql
...
```
##### 二次分组
一旦出现 where 筛选后 GROUP 分组，分组后还需要筛选，马上想到使用 having
...
- 拼接后的表通过 where 筛选出某个字段中哪些符合条件的值会（行）被显示
- SELECT 确定哪些字段（列）**从拼接表中截取出来**显示
- 如果是外连接，拼接后的表保留主表所有内容，副表中没有被主表中匹配填充 NULL
###### 工作原理
1. 内连接（INNER JOIN）
	- 匹配原则：只返回两个表中完全匹配的行。
...
**事务的上下文**：`ROLLBACK` 命令的作用范围是当前事务。`ROLLBACK` 会撤销自上一次 `COMMIT` 后的所有更改。执行 `COMMIT` 所有在该 `COMMIT` 之前执行的语句已经被永久保存到数据库中。还没有 `COMMIT` 都可以通过 `ROLLBACK` 来撤销。
...
SET autocommit=0;
START TRANSACTION;
DELETE FROM account; -- delete执行后不会提交，支持回滚
ROLLBACK;
-- #演示truncate
SET autocommit=0;
START TRANSACTION;
...
END CASE;
```
- Case 结构可以嵌套或放 begin end 中作为独立语句，when 轮空后执行 else 语句
...
- 修改 MySQL 登录密码：
	...
	- 修改配置文件
	- 编辑 MySQL 的配置文件， `/etc/my.cnf` 或 `/etc/mysql/my.cnf`。找到 `[mysqld]` 部分，并添加以下行：可以跳过权限表登录到 MySQL，后操作同上
```file
	[mysqld]
	skip-grant-tables
```
...
#### 索引检索原理
##### 初始化介绍
- b+树，浅蓝色的块我们称之为一个磁盘块，可以看到每个磁盘块包含几个数据项（深蓝色所 示）和指针（黄色所示），
...
- 通过磁盘块 1 的 P 2 指针的磁盘地址把**磁盘块 3 由磁盘加载到内存**，发生第二次 IO，
...
---
### 排名排序
**窗口函数执行阶段在 `GROUP BY` 后、在 `SELECT` 后**；因此 `RANK()`（或其他窗口函数）**不依赖也不产生 GROUP BY**。
...
- **`LEAD(value_expression, offset, default_value) OVER (...)`**: 访问窗口中**当前行后** `offset` 行的 `value_expression` 的值。超出窗口范围的处理方式与 `LAG()` 类似。
...
```
注意 where 会筛选 join 后的表，按行筛选，也就是说，left join 会保留坐标的行，而 on 中筛选条件会筛选掉右边拼接到左边的行中这一部分
...
```
普通索引的快体现在与无索引对比，无索引需要全表扫描，但也有弊端，普通索引的提速效果取决于“查出来的数据量”**。
- **查出的数据量极少（比如 10 条）**：回表 30 次 IO，秒杀全表扫描 → **非常快**。
- **查出的数据量极大（比如 50 万条，占了表的一半）**：回表需要 50 万 * 3 = 150 万次
## 数据库索引是什么？
(https://segmentfault.com/a/1190000018153249)
### 类比于新华字典
数据库索引是从后端初级开发跨越到高级开发的屠龙宝刀。用一句话来描述：**数据库索引就是一种加快海量数据查询的关键技术**。
新华字典的目录就是索引，子的解释信息就是内容，通过目录可以快速知道字在什么位置，这就是索引作用，`INDEX idx_test(col_a, col_b)`。这种包含多个字段的索引就被称为“联合索引”
一个字通过偏旁粗筛后，再通过检字表查找具体的字，这就相当于一个字数据，通过两个索引连接起来，共同作用才能够被检索到。
### 索引类型
索引按 4 个维度分类，同一个索引会同时属于多个维度的多个类型。例如主键索引通常是 B+tree 索引、聚簇索引、单列索引。各维度包含的类型如下：
...
按「数据结构」分类，决定索引的底层查找算法与适用的查询类型：
#### B+tree 索引
B+ 树是多路平衡搜索树，非叶子节点只存键值和指针，叶子节点存数据并按序相连。特点：
- 数据只在叶子节点，非叶子节点可容纳大量键，树高通常 3-4 层，查询稳定在 3-4 次 IO
- 叶子节点按序排列，天然支持范围查询、排序、`LIKE '前缀%'`
- 支持最左前缀匹配
- InnoDB 的聚簇索引和二级索引底层都是 B+tree，是 MySQL 使用最广泛的索引
- 查找过程见[[#索引检索原理]]（3 层树可表示百万数据，只需 3 次 IO）
#### Hash 索引
基于哈希表实现，对索引列的每个值做哈希，等值查询复杂度 O(1)，比 B+tree 更快。维护一个哈希表（数组+链表）。但哈希索引只能做**等值查询（=或IN）**，无法做范围查询（`>` 或 `BETWEEN`），且不排序，MySQL 不支持。特点与限制：
- 只支持等值比较（`=`、`IN`），**不支持范围查询**（`>`、`<`、`BETWEEN`），因哈希后值顺序丢失
- **不支持排序**，`ORDER BY` 无法走哈希索引
- **不支持最左前缀**，哈希针对整列值计算，联合索引的单个字段无法使用
- 存在哈希冲突，冲突多时退化为链表遍历
- MySQL 中 Memory 引擎支持显式 Hash 索引；InnoDB 有自适应哈希索引（Adaptive Hash Index），是 InnoDB 自动在热点页上建立的哈希索引，用户不可手动创建
#### 倒排索引
是全文检索（FULLTEXT）的核心数据结构，B+树是正排索引，倒排索引数据结构为：
1. **单词词典（Term Dictionary）**：存放所有不重复的关键词。这部分通常也用 B+树或哈希表存储，以便快速定位词条。
2. **倒排列表（Posting List）**：每个关键词后面跟着一个列表，里面存着**包含该词的所有文档主键 ID**，以及这个词在该文档中出现的**位置（Position）**和**频率（Term Frequency）**。
假设有两行数据：
...
|---|---|
|苹果|[ (文档 1, 位置 3), (文档 2, 位置 1) ]|
|爱|[ (文档 1, 位置 2) ]|
|手机|[ (文档 2, 位置 2) ]|

**当执行 `SELECT * FROM table WHERE MATCH(content) AGAINST('苹果')` 时**：  
MySQL 直接去倒排索引里找到“苹果”这个词，**立即拿到 [1, 2]**，然后通过这两个 ID 去回表查完整数据。时间复杂度是 O(1) 或 O(log n) 的查找，**完全不用扫描全表**。
#### Full-text 索引
全文检索索引，用于对文本内容做关键词搜索，底层是倒排索引（记录「关键词 → 文档位置，记录“哪个文档里有这个词”」）。MySQL 支持，特点：
- 只能建在 `CHAR`、`VARCHAR`、`TEXT` 类型列上
- 使用 `MATCH (col) AGAINST ('关键词')` 语法，`LIKE '%关键词%'` 无法走全文索引
- MySQL 5.6 起 InnoDB 支持全文索引，5.6 之前只有 MyISAM 支持
- 适合文章、商品描述等大文本检索场景

**按「物理存储」分类**，决定数据行在磁盘上的组织方式：
#### 聚簇索引（主键索引）
叶子节点直接存放整行数据，索引顺序即数据物理存储顺序。InnoDB 中主键索引就是聚簇索引。特点：
- 每张表**至多一个**聚簇索引，因数据只能按一种顺序物理排列
- 通过主键查找最快：定位到叶子节点即拿到整行，无需回表
- 若表未显式定义主键，InnoDB 自动选择第一个非空唯一索引作为聚簇索引，否则生成隐藏的 6 字节 rowid 作为聚簇索引键
- 数据按主键顺序排列：用自增主键插入时追加在末尾，避免页分裂；用随机主键会导致频繁页分裂、性能下降
#### 二级索引（辅助索引）
除聚簇索引外的其他索引，叶子节点只存**索引字段值 + 主键值**，不存整行数据。特点：
- 通过二级索引查找需要先定位到主键值，再**回表**（回到聚簇索引）取整行数据
- 一次二级索引查询 = 二级索引 B+tree 查询 + 聚簇索引 B+tree 查询
- 若查询的列全部包含在二级索引中（覆盖索引），则无需回表，见[[#数据回表和索引下推]]

**按「字段特性」分类**，决定索引的约束能力：
#### 主键索引
`PRIMARY KEY`，要求值非空且唯一，InnoDB 中同时是聚簇索引。每张表只能有一个主键索引，可作用于单列或多列（联合主键）。
#### 唯一索引
`UNIQUE`，要求值唯一，但**允许为空**（可存在多个 NULL，因 NULL 不参与唯一性比较）。一张表可有多个唯一索引。与主键的区别：

| 特性 | 主键索引 | 唯一索引 |
| --- | --- | --- |
| 是否允许为空 | 否 | 是（可多个 NULL） |
| 每表数量 | 至多一个 | 可多个 |
| InnoDB 物理存储 | 聚簇索引 | 二级索引 |
#### 普通索引
`INDEX` / `KEY`，无唯一性要求，仅用于加速查询，是使用最多的索引。
#### 前缀索引
只取字符串列的前 N 个字符建立索引，如 `INDEX idx_name(name(10))`。特点：
- 大幅缩小索引体积，节省空间、提高缓存命中率
- 只能用于 `CHAR`、`VARCHAR`、`BINARY`、`VARBINARY` 等文本类型
- 缺点：无法用于 `ORDER BY` 排序和覆盖索引；前缀区分度低时（如前 10 个字符几乎相同）索引选择性差，需结合数据分布选择合适的前缀长度

**按「字段个数」分类**，决定索引覆盖的列数：
#### 单列索引
只在一个字段上建立的索引，如上文的 `idx_age`。
#### 联合索引（复合索引）
在多个字段上建立的索引，如 `INDEX idx_test(col_a, col_b)`。特点：
- 遵循最左前缀匹配规则，见[[#最左前缀匹配规则]]
- 联合索引本质是「先按 col_a 排序，col_a 相同再按 col_b 排序」的一棵 B+tree，故单独查 col_b 无法使用
- 合理设计可少建索引：一个 `(a, b, c)` 联合索引等效于 `(a)` 和 `(a, b)` 两个索引，但不等效于 `(b)` 或 `(c)`
### 最左前缀匹配规则
...
更简单的说法是：
对于联合索引 `(col1, col2, col3)`，其排序逻辑为：
- **先按col1排序 → col1相同则按col2排序 → col2相同再按col3排序**。
- **从最左字段连续匹配**：索引生效的前提是从第一个字段开始，且中间字段不能跳跃。
- **遇到范围查询截止**：当遇到 `>、<、BETWEEN、LIKE` 等范围条件时，该字段本身可用索引，但后续字段停止使用。
有以下字段，联合索引 `(name, age, salary)`

| 查询条件                                       | 索引使用情况          | 原因分析                               |
| ------------------------------------------ | --------------- | ---------------------------------- |
| `WHERE name='A'`                           | 使用 `(name)`     | 命中第一个字段                            |
| `WHERE name='A' AND age=25`                | 使用 `(name,age)` | 完整匹配前两个字段                          |
| `WHERE name='A' AND age>25`                | 使用 `(name,age)` | `age>25`是范围查询，止步于`age`，`salary`不使用 |
| `WHERE age=25`                             | 不使用             | 跳过最左字段`name`，无法匹配前缀                |
...
- t_order.ibd，t_order 的**表数据**会保存在这个文件。表数据既可以存在共享表空间文件（文件名：ibdata1）里，也可以存放在独占表空间文件（文件名：表名字.ibd）。这个行为是由参数 innodb_file_per_table 控制的，若设置了参数 innodb_file_per_table 为 1，则会将存储的数据、索引等信息单独存储在一个独占表空间，从 MySQL 5.6.6 版本开始，它的默认值就是 1 了，因此从之后，MySQL 中每一张表的数据都存放在一个独立的 .ibd 文件。
### 表空间文件结构
![[表空间结构.drawio.webp]]

记录是按照行来存储的，但是数据库的读取并不以「行」为单位，否则一次读取（也就是一次 I/O 操作）只能处理一行数据，效率会非常低。一般以页作单位读取，每页 16KB
...
### InnoDB 行格式
Redundant 没人用了，5.1 版本后行格式使用 Compact，5.7 之后采用 Dynamic 格式，Dynamic 和 Compress 都由 Compact 衍生而来，除了 Redundant 以外都是紧凑行格式
#### Compact 格式
![[COMPACT.drawio 1.webp]]
*变长字段长度列表*不是必须的，如果表中没有变长字段就没有
对于这样的表数据
```sql
CREATE TABLE `t_user` (
  `id` int(11) NOT NULL,
...
![[变长字段长度列表2.webp]]
![[变长字段长度列表3.webp]]
NULL 值列表长度不固定，但是每个字节可以允许 8 列数据是非 `NOT NULL` 的，每一位记录 NULL 状态，1 表示为 NULL，0 为 `NOT NULL` 同样逆序存放
![[null值列表2.webp]]
...
> 同样，NULL 值列表的数据也需要逆序存放

记录头信息
记录头信息中包含的内容很多，比较重要的有：
- delete_mask ：标识此条数据是否被删除。执行 detele 删除记录的时候，并不会真正的删除记录，只是将这个记录的 delete_mask 标记为 1。
- next_record：下一条记录的位置。从这里可以知道，**记录与记录之间是通过链表组织的**，这是能够进行全表遍历的基础，**指向的是下一条记录的「记录头信息」和「真实数据」之间的位置**，这样的好处是向左读就是记录头信息，向右读就是真实数据，比较方便。
- record_type：表示当前记录的类型，0 表示普通记录，1 表示 B+树非叶子节点记录，2 表示最小记录，3 表示最大记录。
	- **最小记录（Infimum）**：逻辑上代表“比该页中任何用户记录的主键都小”的边界。
	- **最大记录（Supremum）**：逻辑上代表“比该页中任何用户记录的主键都大”的边界。
	- 当全表遍历遍历到当前行数据，**先检索行数据的 record_type**，发现是 3 则终止遍历
## 事务系统
### 事务特性
ACID：
...
| 事务特性 | 说明 | 实现机制 |
|---------|------|---------|
| 原子性 | 一个事务中的所有操作，要么全部完成，要么全部不完成，不会结束在中间某个环节；事务在执行过程中发生错误，会被回滚到事务开始前的状态。 | undo log（回滚日志） |
| 一致性 | 事务操作前和操作后，数据满足完整性约束，数据库保持一致性状态。 | 由原子性、持久性、隔离性共同保证 |
| 隔离性 | 数据库允许多个并发事务同时对其数据进行读写和修改的能力，隔离性可以防止多个事务并发执行时由于交叉执行而导致数据的不一致。 | MVCC（多版本并发控制）或锁机制 |
| 持久性 | 事务处理结束后，对数据的修改就是永久的，即便系统故障也不会丢失。 | redo log（重做日志） |
### 并行事务过程
#### 存在问题
脏读：如果一个事务「读到」了另一个「未提交事务修改过的数据」，简单来说同时执行的 AB 两个事务，B 事务执行过程中读取了不就后 A 事务将会触发回滚覆盖的 A 事务执行过程之间产生的临时数据
![[10b513008ea35ee880c592a88adcb12f.webp]]
不可重复读：在一个事务内多次读取同一个数据，如果出现前后两次读到的数据不一样的情况。原理类似脏读
![[f5b4f8f0c0adcf044b34c1f300a95abf.webp]]
幻读：在一个事务内多次查询某个符合查询条件的「记录数量」，如果出现前后两次查询到的记录数量不一样的情况。类似不可重复读，但是不是值不一样了而是记录条数数量不一样
...
隔离水平：串行化>可重复读>读已提交>读未提交，性能由低到高
不同隔离水平会发生的现象：
![[4e98ea2e60923b969790898565b4d643.webp]]
- 对于「读未提交」隔离级别的事务来说，因为可以读到未提交事务修改的数据，所以直接读取最新的数据就好了，不做任何限制
- 对于「串行化」隔离级别的事务来说，通过加**读写锁**的方式来避免并行访问；
- 对于「读提交」和「可重复读」隔离级别的事务来说，它们是通过 Read View 来实现的，区别在于创建 Read View （类似快照）的时机不同，「读提交」隔离级别是在「每个语句执行前」都会重新生成一个 Read View，而「可重复读」隔离级别是「启动事务时」生成一个 Read View，然后整个事务期间都在用这个 Read View。
#### Read View 实现与工作原理
MVCC = Multi-Version Concurrency Control 多版本并发控制，允许多个事务并发执行且不互相阻塞。
![[readview结构.drawio.webp]]
- m_ids ：指的是在创建 Read View 时，当前数据库中「活跃事务」的**事务 id 列表**，注意是一个列表，**“活跃事务”指的就是，启动了但还没提交的事务**，并且**不包括创建该 Read View 的事务自身**。
- min_trx_id ：指的是在创建 Read View 时，当前数据库中「活跃事务」中事务 **id 最小的事务**，也就是 m_ids 的最小值。
- max_trx_id ：这个并不是 m_ids 的最大值，而是**创建 Read View 时，下一个将要分配给新事务的事务 id 值**（即全局事务 id 计数器的当前值）。
...
MVCC 就是依靠 trx_id 和 min_trx_id/max_trx_id 的比较来运行判断逻辑，roll_pointer 找到回滚的数据
...
## InnoDB 架构与日志系统
> [!Note] 本节定位
> 本节是《MySQL 技术内幕：InnoDB 存储引擎》第 2 章（InnoDB 存储引擎）与第 5 章（redo log）的读书笔记 + 高阶深潜。配合书中图 2-1（InnoDB 逻辑存储架构 / 页缓冲策略）与图 5-1（redo log 工作流程）阅读。
> 本次深潜主线：WAL → log buffer → LogWriter（分歧焦点）→ 刷盘策略 → LSN → checkpoint → 崩溃恢复 → MySQL 8.0 无锁 log 系统。
### 为什么需要 redo log（先从问题出发）
数据库要同时满足两个看似矛盾的需求：
1. **持久性**：commit 的事务数据不能丢，必须最终落在磁盘
2. **性能**：数据页是 16KB 随机位置写入（一次 UPDATE 可能改多个页），随机写磁盘极慢

naive 做法是"每次 commit 都把数据页写盘"，代价是每次提交都有多次随机 IO，吞吐直接被打穿。InnoDB 的解法是 **WAL（Write-Ahead Logging，预写日志）**：
- 数据页先留在内存（buffer pool）里，允许脏页延迟刷盘
- 但 commit 之前，必须先把这个修改的**日志**（追加写、顺序写）落盘

因为日志是**顺序追加写**（磁头不用寻道，SSD 上也是连续的 block），顺序写磁盘速度 ≈ 内存速度的几分之一，远快于随机写。于是用"写日志"代替"写数据页"，把每次提交的随机 IO 换成顺序 IO，这就是 redo log 存在的根本原因。
> [!Warning] 常见误区
> 很多人把 redo log 理解成"数据页的备份"，这是错的。redo log 记录的是**物理层面"页内哪个偏移被改成了什么值"**（page 内的物理修改），而不是整页的副本。它只关心"这次修改对页的字节级影响"，这也是它体积小、能顺序写的根本原因。与之相对，binlog 记录的是逻辑操作（SQL 语句或行变更），这是两者本质区别。
### redo log 的核心概念与架构
#### 三个物理层面：内存、OS、磁盘
InnoDB 的 redo log 数据其实存在于三个地方，理解三分界是理解整个机制的前提：

| 层次      | 位置            | 名称                                                         | 内容                                           |
| ------- | ------------- | ---------------------------------------------------------- | -------------------------------------------- |
| 1 用户态内存 | InnoDB 进程内    | `log buffer`（日志缓冲区）                                        | 事务修改页时产生的 redo 记录，先顺序追加到这里                   |
| 2 内核态缓存 | OS page cache | （无专名，常被误称为"日志文件"）                                          | LogWriter 用 `pwrite` 写入的落点，属于内核缓存，尚未真正刷到磁盘介质 |
| 3 持久化介质 | 磁盘            | redo log 文件（`#ib_redo*`，MySQL 8.0；`ib_logfile*`，MySQL 5.7） | 真正的持久化副本，崩溃后从这里恢复                            |
用户视角只会看到第 1 层和第 3 层，第 2 层是 OS 给所有文件 IO 加的一层透明缓存，这也是全网大量文章出现分歧的根源。
#### log buffer 的结构与写入时机
log buffer 是 InnoDB 用户态的一段环形内存，默认大小由 `innodb_log_buffer_size` 控制（默认 16MB）。**用户事务写 redo 的完整路径**：
```
事务修改 buffer pool 中某页
   │ 1. 页变成脏页（buffer pool 中标记，数据本身还没落盘）
   ▼
mtr（Mini-Transaction）持有页锁，生成 redo 记录
   │ 2. redo 记录先写进 mtr 自己的临时 mtr_buf
   ▼
mtr 提交（释放页锁）时
   │ 3. 整批 redo 拷贝进全局 log buffer（memcpy，快）
   ▼
log buffer 积累（被多个并发 mtr 复用，顺序追加）
```
关键点：**用户事务自己并不直接"刷盘"**。事务执行期间 redo 只是不断往 log buffer 里追加；真正决定"何时把 log buffer 刷出去"的是刷盘策略 + 各种异步触发条件，见下节。这也是"事务写入时 redo 进缓冲区"的正确理解——缓冲区的"缓冲"作用就是合并/攒批多次写入，减少 IO 次数。
#### redo 记录的最小单元：mtr（Mini-Transaction）
mtr 是 InnoDB 内部比事务更小的原子操作单元（一次 B+ 树节点分裂、一行记录插入都算一个 mtr）。一个事务 = 多个 mtr 的集合。设计原因：
- **锁粒度**：页锁（latch）只需要覆盖一个 mtr 的执行过程，不需要覆盖整个长事务，并发度大幅提升
- **原子性粒度**：redo 以 mtr 为粒度保证"要么全部重做要么全部不重做"（mtr 记录里带一个首尾标记，恢复时按 mtr 边界处理）
- **顺序性**：mtr 提交顺序 = redo 写入 log buffer 的顺序 = 页修改顺序，这保证了崩溃恢复时按序重放即正确
### redo log 的持久化与刷盘策略（核心）
#### 刷盘路径：谁写、写到哪、何时写
先给结论，再展开：**LogWriter 线程把 log buffer 写入 OS page cache（第 2 层）；把第 2 层真正刷到磁盘介质（第 3 层）由独立机制完成**。二者是不同层的动作，源码里分别是 `log_writer` 线程负责 `pwrite` 写文件（进 page cache）与 `innodb_flush_log_at_trx_commit` 策略触发的 `fsync`/`O_DIRECT` 直接写盘。
写入路径完整版：
```
log buffer（用户态内存）
   │ LogWriter 线程（独立后台线程，MySQL 8.0）
   │   条件：log buffer 写指针追上刷盘点，或超时（group commit 攒批）
   │   动作：pwrite 到 redo log 文件（此时只进 OS page cache，未落盘介质）
   ▼
OS page cache（内核态）            ←── 这一步被很多文章称为"写日志文件"
   │ 触发条件（满足任一即执行，见下方策略表）：
   │   A. innodb_flush_log_at_trx_commit 策略
   │   B. innodb_flush_log_at_timeout 定时
   │   C. 后台线程（log_flusher）当缓存压力/日志增长时
   │   动作：fsync（或 O_DIRECT 直写）真正落盘
   ▼
磁盘上的 redo log 文件（持久化）
```
#### 分歧点正解：LogWriter 到底是写 page cache 还是写磁盘
**正确答案：LogWriter 只把 log buffer 写入 OS page cache，不保证落盘介质。** 完整证据链：
1. **命名即真相**：MySQL 8.0 把"从 log buffer 写文件"的线程命名为 `log_writer`，把"刷到磁盘"的线程命名为 `log_flusher`，两个线程职责严格分离。如果 LogWriter 已经直接写盘，就不需要第二个线程。
2. **源码证据**：`log_writer` 调用 `log_file_pwrite`（内部走 `pwrite(2)` 写文件），而 `log_flusher` 调用 `log_file_flush`（内部走 `fsync(2)`）。`pwrite` 只是把数据交给内核缓存（page cache），`fsync` 才强制刷到磁盘介质。
3. **行为证据**：若 LogWriter 已直接落盘，`innodb_flush_log_at_trx_commit` 这个"提交时落盘"参数就失去了存在的意义——正是因为它只写缓存，才需要第二个参数控制"何时真正落盘"。
> [!Warning] 为什么全网文章会吵架
> 因为不同文章用的"写盘/落盘/刷盘"三个词含义不同：
> - 说"LogWriter 写盘"的文章，指的是 `pwrite` 那一步——从用户态进入内核态，**严格说只到 page cache**
> - 说"LogWriter 不落盘"的文章，指的是 `fsync` 那一步——**真正写进磁盘介质**
> 两边都不算错，但如果不分清楚这两层，就会把"写 page cache"误当成"已持久化"，这是最危险的误读：page cache 里数据在**操作系统崩溃/断电**时照样丢失。
#### innodb_flush_log_at_trx_commit：提交时落盘策略
| 值 | 行为 | 持久性 | 性能 | 场景 |
| --- | --- | --- | --- | --- |
| `1` | 每次 commit 都 fsync 落盘 | 最强（断电也不丢） | 最慢（每次提交一次 fsync） | **默认值**，对持久性要求高的 OLTP |
| `0` | commit 不落盘，仅由后台线程每秒刷一次 | 弱（断电丢最近 1 秒） | 最快（提交零 IO） | 可容忍丢失 1 秒数据的场景 |
| `2` | commit 时只把日志写进 OS page cache，不 fsync | 中（**OS 崩溃丢，MySQL 崩溃不丢**） | 较快 | 能容忍"整机断电丢数据"、但不能容忍"MySQL 挂"的场景 |
> [!Note] 值 0 与值 2 的本质差异
> 0 和 2 都"省了 fsync"，但语义不同：0 是"提交时**什么都不做**，靠后台每秒兜底刷一次"；2 是"提交时把数据**交给 OS**（pwrite 到 page cache），落不落盘交给 OS"。所以 2 比 0 多了一次"写入内核"的动作，断电时 0 和 2 都会丢（都还没到磁盘介质），但 MySQL 崩溃（进程死，OS 活着）时 2 不丢、0 可能丢。
> 补充：`innodb_flush_log_at_timeout`（默认 1 秒）控制"即使没有事务提交，后台线程也会定期刷 log buffer 到磁盘"，与值为 0 时的兜底机制同源。
#### Group Commit：把多次 fsync 合并成一次
如果每次 commit 都立刻 fsync，高并发下 fsync 次数 = 提交次数，磁盘成瓶颈。解法是 **Group Commit（组提交）**：
- 多个事务几乎同时到达 commit 阶段，第一个事务的 fsync 会等待一小段窗口（`binlog_group_commit_sync_delay`，默认 0 可调）
- 窗口内到达的其他事务共享同一次 fsync：它们的 redo 都已经连续写在 log buffer 里，一次 `fsync` 就能保证**这一批**全部落盘
- 效果：吞吐从"每次提交一次 fsync"变成"每批一次 fsync"，磁盘 IO 次数大幅下降，代价是 commit 延迟增加几毫秒
> [!Note] 3.5 版本的三阶段组提交
> MySQL 5.6 引入、5.7 完整的三阶段组提交（flush、sync、commit 三个阶段，配合 binlog 一起用）是为了协调 redo 与 binlog 的两阶段提交，保证二者的一致性，见[[#崩溃恢复时 redo 与 binlog 如何保持一致（两阶段提交）]]。
### LSN 与 checkpoint
#### LSN：日志序列号
**LSN（Log Sequence Number）** 是 log buffer 里 redo 数据的逻辑字节位置（偏移），单调递增。它同时出现在三处，三处对照理解：
- **页上的 LSN**（`FIL_PAGE_LSN`，每个页头部）：该页**最近一次被修改**时，redo 刷到的 LSN
- **log buffer 的 LSN**（`log_sys.lsn`）：当前日志写到哪个位置
- **文件里的 LSN**（`log_sys.flushed_lsn`）：已经刷到磁盘的日志位置
三者的关系是一个"落后链"：`flushed_lsn ≤ 页上 LSN ≤ buffer 的 lsn`。页的 LSN 小于等于 buffer LSN，说明"这个页的修改日志还在内存里，没落盘"，一旦崩溃需要靠日志恢复；相等则说明日志已安全落盘。
> [!Note] 观察 LSN
> `SHOW ENGINE INNODB STATUS` 输出中有 `Log sequence number` 与 `Log flushed up to` 两行，正是上面说的 buffer LSN 与 flushed_lsn。两者之差就是"还没落盘的日志量"。
#### checkpoint：推进"可安全丢弃的日志"水位
**checkpoint 是一个 LSN 位置，表示该位置之前的 redo log 已经没有用了（对应的页修改都已落盘）**，可以安全覆盖。InnoDB 维护 `checkpoint_lsn`，含义是"所有 LSN 小于它的日志对应的脏页都已刷盘"。
- 当 `checkpoint_lsn` 之后的新日志写入时，会覆盖 checkpoint_lsn 之前的旧日志（redo log 文件是**循环复用**的）
- **checkpoint 推进时机**：由 `log_checkpointer` 线程（MySQL 8.0）周期性调用 `log_consider_checkpoint`，加上两个阈值：
  - `innodb_log_checkpoint_now`：手动触发一次 checkpoint
  - 当"未 checkpoint 的日志量"超过 `innodb_log_checkpoint_max_age`（默认约为日志文件大小的 87.5%）时，强制 checkpoint，防止日志文件被写满
- **checkpoint 过程中要做什么**：把 checkpoint_lsn 之前的所有脏页刷盘，然后写入一个新的 checkpoint 信息（`log_checkpoint_header`，包含 checkpoint_lsn）
> [!Warning] 为什么需要 checkpoint
> 如果不做 checkpoint，redo log 文件会无限增长（日志文件是固定大小循环覆盖的，没有 checkpoint 就不知道该不该覆盖）。而 checkpoint 的另一面是：**checkpoint_lsn 之后的数据页如果崩溃了，必须靠日志恢复**，所以 checkpoint 越靠后，恢复时需要重放的日志就越短，恢复速度越快。
#### 两个 LSN 联动：为什么说 checkpoint 推进 = 让页先落盘
一个常见困惑：日志是"先落盘"，为什么恢复时又要"重放日志"？因为**页本身可能还没落盘**。完整时序：
```
1. 事务修改页 A → 页 A 变脏（页 LSN = 100），redo 记到 LSN 100
2. commit → redo 刷盘到 LSN 100（此时页 A 可能还在内存）
3. 崩溃！
4. 重启：发现页 A 的 LSN (100) > 磁盘上已恢复到的 LSN (0)，
   说明页 A 的修改还没落到磁盘 → 用 redo 从 LSN 0 重放到 LSN 100，恢复页 A
5. checkpoint 推进：把页 A 刷盘（页 LSN 变成磁盘上的值），checkpoint_lsn 前进到 100
   → LSN 100 之前的日志可覆盖，恢复时无需再重放这一段
```
所以 redo 与 checkpoint 是一对"跷跷板"：redo 负责把"日志先落盘"，checkpoint 负责把"数据页补上落盘"，让日志可以循环覆盖。二者共同保证崩溃恢复的正确性与日志空间的循环使用。
### C++ 视角：redo log 的 I/O 与内存管理
> [!Note] 本节将 redo log 机制映射到 C++ 系统编程概念，帮助从"数据库黑盒"跳到"可写代码的模型"。
#### 角色映射表
| InnoDB 概念 | 等价 C++ 概念 / 手法 |
| --- | --- |
| log buffer | 环形缓冲区（`std::vector<byte>` + 头尾游标），多线程追加写 |
| mtr | 一个带锁的作用域对象（RAII：构造加页锁，析构提交 mtr_buf），作用域结束自动释放 |
| LogWriter 线程 | 生产者-消费者模型中的消费者，用条件变量等待"buffer 里有货" |
| redo 记录 | 内存中的结构体（type + space_id + page_no + offset + length + data），序列化后追加写 |
| `pwrite` 写 page cache | `write(2)` 到 O_APPEND/O_DIRECT 打开的 fd（页对齐、顺序写） |
| `fsync` 落盘 | `fdatasync(2)` 或 `O_DIRECT` 绕过 page cache 直写磁盘 |
| LSN | 单调递增的 `uint64_t` 水位，三份拷贝各管一段（页/缓冲/磁盘） |
| checkpoint | 类似"已消费指针"，环形队列里标志"可以安全覆盖"的位置 |
#### 性能关键点（为什么 InnoDB 能这么做）
1. **memcpy 级快**：redo 从 mtr_buf 拷贝到 log buffer 是纯内存操作（可对比 C++ 的 `std::copy` 到预分配缓冲），比"每次写日志都做系统调用"快一个数量级。攒批后再一次 `pwrite`，减少系统调用次数。
2. **顺序写**：redo 是追加式顺序写，天然适合操作系统预读/回写，也适合 O_DIRECT 直写（无需缓存一致性管理）。对比数据页是随机写，这正是 WAL 用顺序写换随机写的核心。
3. **组提交 = 批量 fsync**：类似 C++ 里"批量 flush 输出缓冲（`std::endl` vs `'\n'` 的对比）"——前者每次 flush，后者攒批。Group Commit 就是 InnoDB 版的 `'\n'`。
4. **环形复用 + 水位推进**：log buffer 与 redo 文件都是环形结构，用 LSN（水位）标记可复用位置，类似 C++ 环形队列（`boost::circular_buffer`）的 front/back 指针。
> [!Note] 内存分配策略
> InnoDB 的 log buffer 使用**预分配大块内存**（`log_buffer` 初始化时按 `innodb_log_buffer_size` 一次性分配），避免运行期频繁 `malloc/free` 和页对齐问题；redo 记录写入时如果 log buffer 不够，会触发"先刷一部分再继续写"的阻塞（`log_buffer_reserve` 内处理）。这是典型的"大块预分配 + 水位阻塞"内存管理，与 C++ 高性能服务器常用手法一致。
### MySQL 崩溃恢复机制
#### 恢复的总体流程
崩溃（掉电/OS 崩溃/进程 kill -9）后，InnoDB 重启时做三件事：
```
1. 扫描 redo log 文件，确定恢复起点
   - 从最后一个 checkpoint 记录（log_checkpoint_header）读出 checkpoint_lsn
   - 从 checkpoint_lsn 开始向后扫描 redo，定位到最近一个完整的 mtr（起点要对齐 mtr 边界）
2. 按 LSN 顺序重放（redo）日志
   - 对每一条 redo 记录，找到对应页（space_id + page_no），
     判断该页的当前 LSN 是否小于日志的 LSN
   - 如果小于：说明页还没应用这次修改 → 应用（重做）；否则跳过（页已是最新）
   - 逐条重放，直到 redo 末尾（LSN = flushed_lsn）
3. 收尾：把恢复过程中重新产生的脏页刷盘，推进 checkpoint
```
> [!Note] 恢复为什么"可以重放但不可回滚"
> redo log 是物理日志，重放幂等（同一页同一 LSN 重放多次结果不变），所以恢复只需按序重放即可。未提交事务的修改也在 redo 里（因为 redo 在 commit 前就已经写盘），恢复时**不会回滚未提交事务**——它们靠 undo log（`undo` 里的回滚日志）做逻辑回滚。这是 redo（前滚）与 undo（回滚）的分工。
#### 为什么页 LSN 判断能跳过"已应用"的日志
重放时用 `页 LSN < 日志 LSN` 判断是否应用，这是 redo 恢复的幂等性保证。原因：
- 恢复不是从头到尾全量重放，而是从 checkpoint_lsn 开始
- checkpoint_lsn 之前的日志对应的页**可能已经刷盘**（页 LSN 已等于磁盘值），但也不一定（checkpoint 只保证 checkpoint_lsn 之前的日志**可以丢弃**，不代表所有页都刷了）
- 所以对每条日志都要检查"这条日志是否已经作用在页上"，用页 LSN 与日志 LSN 比较即可判定
#### 崩溃恢复时 redo 与 binlog 如何保持一致（两阶段提交）
InnoDB 在 commit 时协调 redo 与 binlog，保证**崩溃后两日志对事务的判定一致**（要么都认为已提交，要么都认为未提交）：
```
事务提交时：
1. 写 redo（prepare 状态，标记事务处于 prepare）
2. 写 binlog（此时 binlog 落盘）
3. 写 redo（commit 状态，标记事务已提交）
崩溃点在不同位置，恢复结果不同：
- 崩溃在 1 和 2 之间：事务未写 binlog，恢复时回滚
- 崩溃在 2 和 3 之间：事务已写 binlog 但 redo 还是 prepare，
  恢复时**用 binlog 判定**：如果 binlog 里有这条事务，则提交；否则回滚
  （binlog 是权威，因为 binlog 里记录了事务的完整操作）
- 崩溃在 3 之后：事务已 commit，恢复时提交
```
这就是为什么"先写 redo prepare → 再写 binlog → 再写 redo commit"的顺序不能乱：**binlog 是决定事务最终状态（提交/回滚）的权威**，而 redo 只负责"重做"。两阶段提交（2PC）保证主从复制（依赖 binlog）与本地恢复（依赖 redo）看到一致的事务状态。
> [!Warning] 分布式事务与两阶段提交的区别
> 这里的两阶段提交是 **InnoDB 内部的 redo + binlog 协调**，不是分布式事务（XA）的 2PC。XA 是多个数据库实例之间的协调，而这里只是单个实例内两种日志的一致性协议。命名相同、层次不同。
#### 崩溃恢复的快慢因素与优化
- **恢复耗时主要取决于**：从 checkpoint 到日志末尾的距离（即需要重放的日志量）+ 日志对应的页是否在 buffer pool（不在则要读盘）
- **优化方向**：
  - 调大 `innodb_log_file_size` 会减少 checkpoint 频率（减少刷盘），但会**加长恢复时间**（日志更长），两者需要 trade-off
  - MySQL 8.0 引入 `innodb_redo_log_capacity`（日志总容量，默认 100MB），取代了 5.7 的 `innodb_log_file_size` 多文件配置，语义更清晰：容量 = 多个 `#ib_redo*` 文件的总和
### 复盘：两个隐藏问题详解
#### 隐藏问题 1：LogWriter 到底写到了哪一层（分歧总纠偏）
> [!Warning] 一句话结论
> LogWriter 只把 log buffer 写入 **OS page cache**，真正落到磁盘介质由 log_flusher / fsync / O_DIRECT 完成。把"LogWriter 写盘"理解成"直接写磁盘"是错误认识。

**完整链路 + 术语对照**：

| 步骤 | 动作 | 谁执行 | 写入目标 | 是否已持久化 |
| --- | --- | --- | --- | --- |
| 1 | 用户事务修改页 | 用户线程（事务执行者） | buffer pool（页变脏） | 否 |
| 2 | redo 写入 log buffer | mtr 提交时（memcpy） | log buffer（用户态内存） | 否 |
| 3 | log buffer → 文件 | `log_writer` 线程 `pwrite` | **OS page cache** | 否（OS 崩溃丢） |
| 4 | page cache → 磁盘 | `log_flusher` `fsync`（或 O_DIRECT） | **磁盘 redo 文件** | 是 |
| 5 | 数据页 → 磁盘 | 后台刷盘线程（脏页刷盘） | **数据文件** | 是 |
**为什么有人写"LogWriter 直接写盘"也没完全错**：
- 若系统使用 `O_DIRECT` 打开 redo 文件，`pwrite` 会绕过 page cache 直接写磁盘，此时 LogWriter 的那一步确实直接落盘——**但这取决于文件打开方式，不是 LogWriter 的默认行为**
- 默认 `innodb_flush_method`（Linux 为 `fsync`）下，`pwrite` 只到 page cache
**测试验证方法**（在 Linux 上可自行验证）：
```bash
# 查看 redo 文件打开方式（O_DIRECT 还是 O_APPEND）
lsof -p $(pgrep -f mysqld) | grep ib_redo
# 修改 innodb_flush_method 观察 fsync 行为变化
# 用 strace 观察提交时是否出现 fsync 系统调用
strace -f -e trace=fsync,fdatasync -p $(pgrep -f mysqld)
```
#### 隐藏问题 2：redo 与 undo 的分工边界
> [!Warning] 一句话结论
> redo 负责**前滚**（重做已提交事务的修改），undo 负责**回滚**（撤销未提交事务的修改）。二者在崩溃恢复中配合，但机制完全不同。

| 维度 | redo log | undo log |
| --- | --- | --- |
| 记录内容 | 物理修改（页内偏移 → 新值） | 逻辑反操作（旧值 / 反 SQL） |
| 作用方向 | 前滚（恢复已提交的修改） | 回滚（撤销未提交的修改） |
| 写入时机 | 修改页之前（WAL） | 修改页之前（记录旧值） |
| 崩溃恢复时 | 从 checkpoint_lsn 重放到末尾 | 扫描 undo 页，对未提交事务执行回滚 |
| 生命周期 | 循环覆盖（checkpoint 后丢弃） | 事务结束（提交/回滚）后清理 |
| 是否必须落盘 | 是（commit 前） | 否（可延迟，靠刷脏页时顺带落盘） |
**为什么 undo 不需要像 redo 一样"commit 前强制落盘"**：
- redo 是"已提交事务"的恢复依据，commit 后必须保证可恢复 → 必须提前落盘
- undo 是"未提交事务"的回滚依据，崩溃时这些事务本来就要回滚（数据不生效），所以 undo 的落盘可以推迟到刷脏页时（脏页刷盘前需要先写 undo，否则脏页一旦落盘，未提交的修改就没法回滚了——这个顺序是"undo 先落盘、脏页后落盘"）
> [!Note] 事务内部流程串起来
> 一个完整 UPDATE 事务内部：
> 1. 写 undo（记录旧值，为回滚做准备）
> 2. 修改 buffer pool 中的页（页变脏）
> 3. 写 redo（记录物理修改，WAL）
> 4. commit → redo 刷盘（落盘策略控制）
> 5. 后台：脏页刷盘（需先确保 undo 已落盘）+ checkpoint 推进
> 6. 日志循环覆盖，undo 在事务结束后清理
### 高频面试题自查
1. 为什么 redo log 能显著提升写入性能？（WAL：用顺序写日志代替随机写数据页）
2. redo log 和 binlog 的区别？（物理日志 vs 逻辑日志，恢复 vs 复制，InnoDB vs Server 层）
3. 事务提交时 redo 和 binlog 如何保证一致性？（两阶段提交，binlog 为权威）
4. 为什么崩溃恢复时可以"重放"但不能"回滚"未提交事务？（redo 物理日志幂等，未提交靠 undo）
5. checkpoint 的作用？（让日志可循环覆盖 + 缩短恢复时间）
6. `innodb_flush_log_at_trx_commit=0/1/2` 的区别与适用场景？
7. LogWriter 写日志文件时是写到 page cache 还是磁盘？（page cache，fsync 才落盘）