...
| /usr/share/   | 放置共享文件的地方，在这个目录下放置的数据几乎是不分硬件架构均可读取的数据， 因为几乎都是文本文件嘛。在此目录下常见的还有这些次目录：/usr/share/man：联机帮助文件<br><br>/usr/share/doc：软件杂项的文件说明<br><br>/usr/share/zoneinfo：与时区有关的时区文件                                                |
