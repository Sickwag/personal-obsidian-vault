...
# 上面代码对word中的重复字母使用了两次.count方法,但是在对一个重复的字母使用时已经得到重复次数的答案,使用set剔除重复元素
# 对上面代码优化
>>> word = 'letters'
...
**none和false在布尔值中是一样的** ,但是本质不一样
你需要把 None 和不含任何值的空数据结构区分开来。0 值的整型 / 浮点型、空字符串（''）、空列表（[]）、空元组（()）、空字典（{}）、空集合（set()）都等价于 False，但是不等于 None。
```python
>>> is_none(None)
It's None
>>> is_none(True)
...
`yield` 关键字用于定义一个生成器函数，而不是一个独立的函数。生成器函数允许你声明一个函数，它能够按需产生一系列的值，而不是一次性返回一个完整的值列表。这使得生成器函数在处理大量数据时非常有用，因为它可以节省内存。
...
yield相当于一个return语句,是一种返回返回值的方式, 需要创建一个可迭代序列时,正常方式是计算出( 返回出 )每一个返回值,并使用append添加到序列号中, yield会每次生成一个可迭代对象中的元素, 最后组成一个序列,节省内存,提高速度.

## 装饰器
- 装饰器分为内外层, 外层函数不执行任何代码, 仅仅是将需要被装饰的参数传入内部函数., 
- 定义好一个装饰器之后,任何传入装饰器中的函数都会被装饰器的内层函数装饰在本身的前后.
...
- 返回值：返回值是对象的唯一标识符，通常情况下，这个标识符是对象在内存中的地址。
...
## 包和模块
**random**模块中的`choice`方法
`choice(数据容器)`会从数据容器中抽取一个元素作为语句输出

**包**的本质是一个文件夹,但是文件中的`__init__.py`文件夹会被解释器认为这个文件是包
...
d_dict = defaultdict(lambda: {}) # 因为python没有字典关键字
d_dict['c']['key'] = 'value'
...
2. 遍历列表 `['spam', 'spam', 'eggs', 'spam']`，对于列表中的每个元素 `food`：
...
`tertools` 是 Python 标准库中的一个模块，它提供了一系列用于创建和使用迭代器的工具。这些工具可以用来高效地处理数据流，组合多个迭代器，以及生成复杂的迭代模式。

`itertools` 模块中的迭代代码结构通常用于创建无限或有限的迭代器序列，这些序列可以用于循环、数据处理、组合生成等场景。
...
> 
> 例如，整数 7 就是一个**包含了加法、乘法之类方**法的整数类的对象，可用`.`使用整数类中的方法. 字符串 'cat' 和 'duck' 也是字符串对象，它们都包含着 capitalize() 和 replace() 之类的字符串方法。
> 
...
### 使用命名重整防止访问外部特性
在类中命名特性时,前面加双下划线即可防止访问, 这种命名规范本质上并没有把特性变成私有，但 Python 确实将它的名字重整了. 使用  下划线 + 类名 +双下划线+特性明 仍然可以访问
```python
>>> fowl = Duck('Howard')
>>> fowl.name
inside the getter
...
类中的属性可以被成员方法调用,但是调用类不会调用成员方法中的变量. 其实本质上这是一种**层级关系**
```python
class A():
    count = 0  # 类属性
    time = 1054  # 类属性
...
object1 = A.exclaim()  # 在调用exclaim方法时其中的便来你刚才会被创建
object2 = A.stuff()  # 同理,但是和上面互不干涉而是用
class1 = A()    # 可以访问类中的所有变量和方法,即class1.exclaim,而上面的Object加点只能访问其管辖域的内容
```
...
但是由于直接调用对象作为参数,可以起到对不同类中重复方法名一种类似继承的调用
...
"""代码报错,因为Unicode 字符名称索引页列出的字符名称是经过修改的，因此与由 name() 函数得到的名称有所不同。如果需要将它们转化为真实的 Unicode 名称（**Python 使用的**），只需将逗号舍去，并将逗号后面的内容移到最前面即可。"""

>>> >>> unicodedata.lookup('LATIN SMALL LETTER E WITH ACUTE')
'é'
```
得到编码,就得到了字符在Unicode中的具体位置, 是一个使用两字节,字节之间用十六进制表示的四位八进制数( 两位十六进制数 ), \u表示十六进制
...
在上面占位符之后添加`% (变量名1 , 变量名2 )`根据占位格式进行插值操作
...
对二进制数据进行逻辑运算时是以其二进制数据表示之后按位运算得到结果的
...
### seek()和tell()改变和读取位置
Python 都会跟踪文件中的位置。函数` tell() `返回距离文件开始处的字节偏移量。函数` seek() `允许跳转到文件其他字节偏移量的位置。
...
- `restkey`：如果某些行的列数少于 `fieldnames` 中的字段数，剩余的**字段**将被归入一个列表，并以 `restkey` 作为键名。
- `restval`：如果某些行的列数少于 `fieldnames` 中的字段数，剩余的**字段**将被赋予 `restval` 的值。
- `extrasaction`：如果某些行的列数多于 `fieldnames` 中的字段数，可以设置为 `'raise'` 来抛出异常，或者 `'ignore'` 来忽略额外的列。
...
**不接受任何参数**的同时, 只接受`dictreader`对象作为调用对象,只能在其处理之后调用写入表头
