...
在前面，我们学习了许多的Linux命令和高级技巧，这些知识点比较零散，同学们跟随着课程的内容进行练习虽然可以基础掌握这些命令和技巧的使用，但并没有一些具体的实操能够串联起来这些知识点。
...
> rabbitmq在yum仓库中版本比较老，所以我们需要手动构建yum仓库
...
14. 格式化NameNode，在node1执行
    ...
    > hadoop这个命令来自于：$HADOOP_HOME/bin中程序
    >
...
15. 启动hadoop的hdfs集群，在node1执行即可
    ...
    > start-dfs.sh这个命令来自于：$HADOOP_HOME/sbin中程序
    >
...
2. `Grafana`官方托管. 无需安装, 在线注册即可得到一个专属于自己的`Grafana`, 但要花钱的. 是一种`SaaS`服务
