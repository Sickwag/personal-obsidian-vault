...
> 来源：AcWing02 - 01背包问题 - https://www.a
>
> cwing.com/problem/content/description/2/
...
因此，结果就是两个划分中最大值，即 $dp(i,j)=\max\left\{dp(i-1,j),dp(i-1,j-v_i)+w_i\right\}$
...
但直接改为滚动数组还有一个问题，第二层循环计算到 $j$ 的时候，比 $j$ 小的数据已经全部被新数据覆盖了，因 $j-v[i]<j$，所以 $dp[i - 1][j - v[i]]$ 这个值已经被覆盖为了 $dp[i][j - v[i]]$。
...
思想和上面的 01 背包一模一样。但此处不能反向循环，得正向循环，因 $dp[i][j]$ 使用的都是同层的数据，正向可以取到正确数据，反向的话就取到了上一层 dp 的数据。
