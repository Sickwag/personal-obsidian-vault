...
**意思就是跳出了最内层的循环后第二层还在计算，计算到只用**1**角和**2**角不用**5**角的结果再次符合最内层的**if**条件后在输出结果**
**代码优化（使用接力**break**语句）**
```C
\#include <stdio.h>
int main()
...
- 函数的声名可以写在程序文件的任意位置
    ...
    void print（）;//函数原型提前声明，告诉编译器我有这个函数，你先不要报错，编译时，你去下面找

    放在调用函数代码之前的地方

    在函数下次调用时就会识别到

- 在函数中调用函数时，不需要写上函数的类型，不然int printf()函数就会被编译器认为实在函数中声明函数，调用只需写上名字就可以
...
最后的最后，当输入的数字等于1（x=1）就会导致dowhile循环中循环体至少循环一次，从而导致，mask至少是10，而且因有两轮循环的原因，x已经在第一轮循环后改变了值，所以需要设置新的变量
```C
错误
\#include<stdio.h>
int main()
...
- 任何一个数%n，得到的结果是[0，n-1]的整数，所以使用rand()得到随机数后然他生成[1,100]的数字，当然，也可以直接写int number = rand()%101
- [[rand函数]]
...
- 解决方法是将scanf**读取数字后**使用if判断是否要继续放入循环中计算
```C
\#include<stdio.h>
int main() {
